...
handler.post(async (req, res) => {
    try {
        ...
        let result = await userService.addCreditCard(req.user, name, number, expiry, cvc);
        await db.disconnectDb();
        // console.log("resultApi", result);
        if (result != null && result != "undefined") {
            return res.status(200).json({ message: "Карту додано успішно!", creditCards: result.creditCards });
        } else {
            res.status(400).json({ error: "Помилка додавання картки!" })
        };
...
handler.put(async (req, res) => {
    try {
        ...
        await userService.removeFromWishlist(req.user, name, number, expiry, cvc, issuer);
