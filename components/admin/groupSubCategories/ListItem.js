...
export default function ListItem({
  ...
  const handleRemove = async (id) => {
    try {
      const { data } = await axios.delete("/api/admin/groupSubCategory", {
        data: { id },
      });
  ...
  return (
    <li className={styles.list__item}>
      ...
      {open && (
        <div className={styles.list__item_expand}>
          ...
          <button
            ...
            onClick={() => handleUpdate(groupSubCategory._id)}
          >
            Зберегти
          </button>
          <button
      ...
      <div className={styles.list__item_actions}>
        ...
        <AiFillDelete onClick={() => handleRemove(groupSubCategory._id)} />
      </div>
    </li>
  );
}
