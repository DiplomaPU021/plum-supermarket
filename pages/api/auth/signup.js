...
handler.post(async (req, res) => {
  try {
    ...
    if (!validateEmail(email)) {
      return res
        .status(400)
        .json({ message: "Введіть дійсну електронну адресу" });
    }
    ...
    if (password.length < 6) {
      return res
        .status(400)
        .json({ message: "Пароль має містити принаймі 6 літер" });
    }
    ...
    res.json({
      message:
        "Успішно зареєстровано! Перевірте вашу адресу на наявність листа активації",
    });
  } catch (error) {
