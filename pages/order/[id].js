...
export default function OrderId({ country, orderData, user }) {
    ...
    <Container fluid className={styles.cont}>
      ...
      <Row className={styles.row}>
        <div className={styles.repete}>
          <Link type="button" href="/" className={styles.light_button}>
            Продовжити покупки
          </Link>
          {/* TODO перехід на мої замовлення профіль*/}
          <MyCabinet
            show={showMyCabinet}
            onHide={() => setShowMyCabinet(false)}
            user={user}
            orders={orderData}
            country={country}
          />
          <button className={styles.dark_button} onClick={handlerMyOrders}>
            Мої замовлення
          </button>
        </div>
      </Row>
