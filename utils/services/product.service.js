...
const findByReviewByAndUpdate = async (
  ...
  if (product) {
    const exist = product.reviews.findIndex(
      (x) => x.reviewBy.toString() === userId,
    );
    if (exist != -1) {
  ...
  try {
    ...
    for (let j = 0; j < products.length; j++) {
      ...
      for (let i = 0; i < product.subProducts.length; i++) {
        ...
        sizeIndex = subProduct.sizes.findIndex(
          (size) => size.code?.toString() === products[j].code?.toString(),
        );
        if (sizeIndex !== -1) {
          subProductIndex = i;
          break;
      ...
      if (subProductIndex === -1 || sizeIndex === -1) {
        throw new Error(
          `Sub-product with size code ${products[j].code} not found`,
        );
      }

      // Зменшуємо кількість товару на величину quantity
      product.subProducts[subProductIndex].sizes[sizeIndex].qty -=
        products[j].qty;
      if (isPaid) {
