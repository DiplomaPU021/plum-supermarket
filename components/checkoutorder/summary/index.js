...
export default function Summary({
  ...
  useEffect(() => {
    ...
    if (delivery.deliveryId == "postmanDelivery") {
      setTotalAfterDiscount(
        (
          ((100 - discount) * getTotalPrice()) / 100 +
          Number(delivery.deliveryCost)
        ).toFixed(),
      );
    } else {
      setTotalAfterDiscount(
        (((100 - discount) * getTotalPrice()) / 100).toFixed(),
      );
    }
    ...
    if (session) {
      if (
        ...
        try {
          ...
          console.error("summary129", error);
        }
      } else {
  ...
  return (
    <div>
          ...
          <Form>
            <div className={styles.confirm}>
              ...
              <Button
                ...
                onClick={() =>
                  setShowPromo(showPromo === "none" ? "block" : "none")
                }
              >
                Промокод
                {showPromo === "none" ? (
              ...
              <div className={styles.promo_div} style={{ display: showPromo }}>
                <Form.Control
                  ...
                  isInvalid={
                    (formik.touched.promocode && couponError != "") ||
                    (formik.touched.promocode && !!formik.errors.promocode)
                  }
                  isValid={
                    formik.touched.promocode &&
                    couponError === "" &&
                    formik.initialErrors.couponError == ""
                  }
                />
                <Form.Control.Feedback type="invalid">
                  {formik.errors.promocode || formik.initialErrors.couponError}
                </Form.Control.Feedback>
                <Form.Control.Feedback type="valid"></Form.Control.Feedback>
                {visible ? (
                  <Button
                    className={styles.small_sbm}
                    onClick={(e) => applyCouponHandler(e)}
                  >
                    Застосувати
                  </Button>
                ) : (
              ...
              <Row className={styles.total}>
                ...
                <Col lg={7} className={styles.details_data}>
                  <span>
                    <h3>
                      {Math.round(totalAfterDiscount).toLocaleString("uk-UA")} ₴
                    </h3>
                  </span>
                </Col>
                <Button
                ...
                />
                <Form.Control.Feedback type="invalid">
                  {formik.errors.orderError?.userError ||
                    formik.errors.orderError?.shippingError ||
                    formik.errors.orderError?.paymentError}
                </Form.Control.Feedback>
              </Row>
              <div className={styles.form_line}></div>
              <div className={styles.info}>
                <p>
                  Отримання замовлення від 5 000 ₴ тільки за паспортом (Закон
                  від 06.12.2019 № 361-IX)
                </p>
                <ul>
                  Підтверджуючи замовлення, я приймаю умови:
                  <li>
                    <div className={styles.litext_btn}>
      ...
      <PersonalDataPolicy show={infoShow} onHide={() => setInfoShow(false)} />
      <UserConditions show={info2Show} onHide={() => setInfo2Show(false)} />
    </div>
  );
}
