      ...
      <Row className={styles.orderitem}>
        {showFulllOrder === "none" && !props.adminActive ? (
          <Col xs={4} className={styles.col}>
            <p>
              №{props.order._id.substring(0, 6)} від{" "}
        ...
        {showFulllOrder === "none" && !props.adminActive ? (
          <Col xs={3} className={styles.col}>
            <p>
              ...
              <br />
              <span>
                {props.order.costAfterDiscount.toLocaleString("uk-UA")} ₴
              </span>
            </p>
          </Col>
        ) : (
        ...
        <Col xs={1} className={styles.col}>
          <button
            onClick={() =>
              setShowFulllOrder(showFulllOrder === "none" ? "block" : "none")
            }
          >
            {showFulllOrder === "none" ? (
              <img
        ...
        <Col className={styles.ordertable}>
                ...
                {props.order.products.map((product, i) => (
                  <tr key={i}>
                    ...
                    <td className={styles.td}>
                      {product.name.substring(0, 40)}... {product.color?.color}{" "}
                      {product.size}
                    </td>
                    <td className={styles.centered}>{product.qty}</td>
                    <td className={styles.centered}>
                      {(product.priceAfter * product.qty).toLocaleString(
                        "uk-UA",
                      )}{" "}
                      ₴
                    </td>
          ...
          <Row className={styles.deliveryinfo}>
            <div className={styles.delivery_item}>
              <p>Отримувач</p>
              <span>
                {props.order.shippingAddress?.firstName}{" "}
                {props.order.shippingAddress?.lastName}
              </span>
            </div>
            <div className={styles.delivery_item}>
              <p>Оплата</p>
              {props.order.discount > 0 ? (
                <div>
                  <span>Застосовано промокод {props.order.promocode}</span>
                  <br />
                  <span>Знижка {props.order.discount}%</span>
                  <br />
                </div>
              ) : (
                <></>
              ...
              <span>
                ...
                <b>{props.order.costAfterDiscount.toLocaleString("uk-UA")} ₴</b>
              </span>
              <span>
                Статус: {props.order.isPaid ? "Оплачено" : "Очікується оплата"}
              </span>
            </div>
            <div className={styles.delivery_item}>
              <p>Доставка</p>
              ...
              <span>
                ...
                {props.order.deliveryMethod.deliveryId !== "postmanDelivery" ? (
                  <span>
                    <small>{props.order.deliveryMethod.deliveryCost}</small>
                  </span>
                ) : (
                  <span>
                    <b>{props.order.deliveryMethod.deliveryCost} ₴</b>
                  </span>
                )}
              </span>
              {props.order.deliveryMethod.deliveryId === "postmanDelivery" ? (
                <span>
                  <small>{props.order.shippingAddress?.address}</small>
                </span>
              ) : props.order.deliveryMethod.deliveryId === "selfPickup" ? (
                <span>
                  Адреса магазину:{" "}
                  <small>{props.order.deliveryMethod.deliveryAddress}</small>
                </span>
          ...
          {!props.adminActive ? (
            <div className={styles.repete}>
              ...
              <button className={styles.light_button} onClick={handleFeedBack}>
                Залишити відгук
              </button>
              <button
